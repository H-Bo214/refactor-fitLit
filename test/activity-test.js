...
describe('Activity', function() {
  let activityData, user1, user2, user3, users, userRepo, activity;
  ...
  it('should return the number of minutes a given user was active for on a given day', function() {
    expect(activity.getActiveMinutesByDate(11, '2019/06/16')).to.eql(106);
  });
  ...
  it('should return average steps taken for given date for all users', function() {
    expect(activity.getAllUsersAverageDataForDay('2019/06/15', userRepo, 'numSteps')).to.eql(6250)
  });

  it('should return average minutes active given date for all users', function() {
    expect(activity.getAllUsersAverageDataForDay('2019/06/15', userRepo, 'minutesActive')).to.eql(108.8)
  });
  ...
  it('should return minutes active for given user on given date', function() {
    expect(activity.getUserDataByDate(11, '2019/06/16', userRepo, 'minutesActive')).to.eql(106);
  });

  it('should return a weeks worth steps for a given user', function() {
    expect(activity.getUserDataForWeek(11, '2019/06/17', userRepo, 'numSteps')[2]).to.eql('2019/06/15: 2000');
  });

  it('should return a weeks worth active minutes for a given user', function() {
    expect(activity.getUserDataForWeek(11, '2019/06/17', userRepo, 'minutesActive')[1]).to.eql('2019/06/16: 106');
  });
  ...
  it('should get a users friend lists activity', function() {
    ...
    expect(activity.getFriendsActivityData(userRepo.users[0], userRepo)).to.eql(result)
  });

  it('should get a users ranked friendslist activity for a chosen week', function() {
    expect(activity.getFriendsAverageStepsForWeek(userRepo.users[1], '2019/06/17', userRepo)).to.eql([{ '1': 10000 }, { '3': 9000 }, { '11': 3667 }]);
  });

  it('should get a users ranked friendslist activity for a chosen week with names', function() {
    expect(activity.displayStepChallengeWinner(userRepo.users[0], '2019/06/15', userRepo)).to.eql([
      'The Rock: 9000', 'Allie McCarthy: 4000'
    ])
  });

  it('should know the ID of the winning friend', function() {
    expect(activity.getWinnerById(userRepo.users[1], '2019/06/15', userRepo)).to.eql(1)
  });

  it('should show a 3-day increasing streak for a users step count', function() {
    expect(activity.displayIncreasedSteps(userRepo, 11, 'numSteps')).to.eql(['2019/06/17'])
  });

  it('should show a 3-day increasing streak for a users minutes of activity', function() {
    expect(activity.displayIncreasedSteps(userRepo, 11, 'minutesActive')).to.eql(['2019/06/17'])
  });
});